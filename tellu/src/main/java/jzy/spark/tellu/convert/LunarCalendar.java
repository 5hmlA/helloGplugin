...
public final class LunarCalendar {
    ...
    static {
        MONTH_STR = Utills.getContext().getResources().getStringArray(R.array.lunar_first_of_month);
        TRADITION_FESTIVAL_STR = Utills.getContext().getResources().getStringArray(R.array.tradition_festival);
        DAY_STR = Utills.getContext().getResources().getStringArray(R.array.lunar_str);
        SPECIAL_FESTIVAL_STR = Utills.getContext().getResources().getStringArray(R.array.special_festivals);
        SOLAR_CALENDAR = Utills.getContext().getResources().getStringArray(R.array.solar_festival);
    }
     ...
     * @return 公历节日
     */
    public static String gregorianFestival(Calendar calendar) {
        return gregorianFestival(calendar.getMonth(), calendar.getDay());
    }
    ...
    public static String getSolarTerm(Calendar calendar) {
        return getSolarTerm(calendar.getYear(), calendar.getMonth(), calendar.getDay());
    }
...
//        return LunarCalendar.numToChinese(lunar[1], lunar[2], lunar[3]);
     ...
     * @return 获取西方节日
     */
    private static String getSpecialFestival(Calendar calendar) {
        return getSpecialFestival(calendar.getYear(), calendar.getMonth(), calendar.getDay());
    }
    ...
    public static String getHoliday(){
        ...
        //农历节日
        String lunarText = getLunarText(calendar);
        if (!TextUtils.isEmpty(lunarText)) {
            return lunarText;
        }
        //24节气
        String solarTerm = getSolarTerm(calendar);
        if (!TextUtils.isEmpty(solarTerm)) {
            return solarTerm;
        }
        //西方特殊节日
        String specialFestival = getSpecialFestival(calendar);
        if (!TextUtils.isEmpty(specialFestival)) {
            return specialFestival;
        }
        //新历节日
        String festival = gregorianFestival(calendar);
        if (!TextUtils.isEmpty(festival)) {
            return festival;
        }
        return "";
